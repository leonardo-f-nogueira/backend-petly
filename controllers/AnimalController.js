...
exports.cadastrarAnimal = async (req, res) => {
  const { 
    name, species, breed, age, size, gender, 
    description, status, behavior, healthIssues, photoUrl 
  } = req.body;
  
  const abrigoId = req.user.id; 

  if (!name || !species || !age || !size || !gender || !photoUrl) {
    return res.json({ erro: "Campos obrigatórios (Nome, Espécie, Idade, Porte, Gênero, Foto) não preenchidos!" });
  }

  try {
    const novoAnimal = await Animal.create({
      name, species, breed, age, size, gender, 
      description, status: status || 'Disponível',
      behavior, healthIssues, photoUrl,
      abrigoId: abrigoId 
    });
...
exports.atualizarAnimal = async (req, res) => {
  ...
  try {
    ...
    if (!animal) {
      return res.status(404).json({ erro: "Animal não encontrado ou não pertence a este abrigo!" });
    }
